...
## 2026-06-12 — Continuous Growth 架构定调
...
| 优先级 | 任务 | 状态 |
|--------|------|------|
| 🔴 | 修 hybrid_search.py import 错误 | 待实施 |
| 🔴 | 建 store/database.py（统一连接管理） | 待实施 |
| 🔴 | 建 events 表 + core/journal.py | 待实施 |
| 🟡 | 实现 core/context.py（分层注入 + Token 预算） | 待实施 |
| 🟡 | 重构 brain.py → talk() | 待实施 |
| 🟡 | 拆分 api/chat.py 为多资源路由 | 待实施 |
| 🟢 | 实现离线 Evaluator | 待实施 |
| 🟢 | 前端渐进改造 | 待实施 |
| ⚪ | 清理死代码（planner/learn/训练面板） | 待实施 |
